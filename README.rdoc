...
1. In most languages, this will fail:
    if ((0.1 + 0.2) == 0.3)
      # Do something here
    end
   Why?

2. Generally, quicksort is more efficient than bubblesort. Under what circumstances would you prefer bubblesort?

3. Why might you denormalise your database?

4. Which is the correct order, from fastest to slowest?
...
5. How would you store passwords in your database in such a way that they’d remain secure if the database were to be obtained by hackers?


6. What would be the output of the following pseudocode?
...
8. You have database tables that look like this:
    CREATE TABLE `users` (
